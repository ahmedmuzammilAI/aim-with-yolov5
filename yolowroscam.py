...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    ros = source.lower().startswith(('ros'))
    ...
    try:
        print("trying to open arduino")
        arduino = serial.Serial('COM11', baudrate=9600)
    except:
        print("check the port.. ok nvm doesnt matter if u do ")
    ...
    for path, img, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        (h, w) = im0.shape[:2]
                        h1,w1=h // 2,w // 2
                        cv2.circle(im0, (w1, h1), 7, (255, 255, 255),-1)  # where w//2, h//2 are the required frame/image centeroid's XYcoordinates.
                        ...
                        c1 = (x1 + x2) / 2
                        c2 = (y1 + y2) / 2
                        cv2.circle(im0, (int(c1), int(c2)), 7, (255, 0, 0),-1)  # where w//2, h//2 are the required frame/image centeroid's XYcoordinates.
                        ...
                        # centre = [c1, c2]
                        # arduino
                        # arduino = serial.Serial('COM3', baudrate=9600) line 146
                        # arduino.write(centre)

                        c3 = int(c1)
                        c4 = int(c2)
                        cstr=str(round(c1))+str(round(c2))+" "
                        #print("centre in str:", cstr)
                        #arduino.write(bytes(c3))
                        #arduino.write(b'abcdef\n')
                        ...
                        try:
                            if arduino:
                                arduino.write(bytes(cstr.encode()))
                                fromard=arduino.read()
                                print("fromard:", fromard)
                        except:
                            pass
                        if save_crop:
            ...
            if view_img:
                (h, w) = im0.shape[:2]
                cv2.circle(im0, (w // 2, h // 2), 7, (255, 255, 255), -1)
                # cv2.PutText()
...
def rosimage():
    ...
    ic = yolo()
    try:
        # r = rospy.Rate(1)  # 30Hz change laterf
...
def talker(x,y):
    pub = rospy.Publisher('centre', Int32MultiArray, queue_size=10)
    rospy.init_node('coord', anonymous=True)
    rate = rospy.Rate(10) # 10hz
    centre = Int32MultiArray()

    if not rospy.is_shutdown():
        try:
            centre.data = [x,y]
            rospy.loginfo(centre.data)
            print(centre.data[0],"--",centre.data[1])
            pub.publish(centre)
            rate.sleep()
        except KeyboardInterrupt:
            cv2.destroyAllWindows()
